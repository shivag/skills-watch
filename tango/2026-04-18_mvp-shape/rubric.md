...
- [ ] **[PAINKILLER]** On a purpose-built red-team skill, the universal deny-list blocks 100% of these attacks **by default**, terminates the skill process immediately, and reports `BLOCKED`: (1) read `~/.ssh/id_rsa`, (2) `curl` to a non-allowlisted host (e.g. `example.com`), (3) `pip install <pkg>` mid-run, (4) write to `~/.zshrc`, (5) read `~/.zsh_history` or `~/.bash_history` to exfil shell-history-leaked secrets. *Verifiable by `spikes/redteam_<N>.py` producing a BLOCKED verdict for each.*
...
- [ ] **[CLARITY]** Every `BLOCKED` line is actionable: it states exactly what was blocked AND points to future recourse. Format: `BLOCKED: [ACTION] [OBJECT] — to allow in a future version, see https://skills-watch.dev/docs/allowlist`. *Verifiable by format spec; turns a hard "no" into a "not yet" and collects demand for v1 allow-listing.*
...
- [ ] Universal filesystem deny-list covers at minimum: `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.netrc`, `~/.docker/config.json`, `~/.agents/.env`, `~/.zsh_history`, `~/.bash_history`, any `.env` outside the skill's CWD. *Verifiable by unit test against each path.*
